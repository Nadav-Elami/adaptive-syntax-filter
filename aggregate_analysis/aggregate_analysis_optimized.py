#!/usr/bin/env python3
"""Optimized Aggregate Statistics Analysis for Adaptive Syntax Filter.

FIXES:
1. Uses pool.imap_unordered() to prevent ordering deadlocks
...
Usage:
    python aggregate_analysis_optimized.py --config-id 2 --n-seeds 50000 --parallel 64
"""
...
class OptimizedAggregateAnalysisManager:
    """Optimized version that fixes multiprocessing deadlocks."""
    ...
    def run_parallel_batch_optimized(self, config_id: int, n_seeds: int, n_processes: int = 4,
                                   resume: bool = False) -> List:
        """Optimized parallel processing that prevents deadlocks."""
...
def main():
    """Main entry point with optimized processing."""
    parser = argparse.ArgumentParser(
        description="Optimized Aggregate Analysis (fixes multiprocessing deadlocks)",
    ...
    logger.info("🚀 STARTING OPTIMIZED AGGREGATE ANALYSIS")
    ...
    manager = OptimizedAggregateAnalysisManager(output_dir=args.output_dir)
    
    start_time = time.time()
    
    # Run optimized parallel processing
    results = manager.run_parallel_batch_optimized(
